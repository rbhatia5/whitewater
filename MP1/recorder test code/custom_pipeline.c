...
Returns		: T/F
*/
static void assemble_pipeline_with_record()
{
	gboolean ret = TRUE;
	...
	{
		ret = FALSE;
		g_print("Linking elements failed.\n");
	}
	
	data.custom_bin = GST_BIN(data.pipeline);
}

static void assemble_pipeline_without_record()
{
	gboolean ret = TRUE;
	data.pipeline = gst_pipeline_new("test-pipeline");
	if(!data.pipeline)
